 ...
 */
export const sendCredentials = (cleanedScanResult, camera) => async dispatch => {
	...
	if (cleanedScanResult.length) {
		...
		.then((res) => {
			...
			let data = {
				...
				accessToken: res.data && res.data.access_token ? res.data.access_token : cleanedScanResult,
