...
import hardcodedTestQuestionnaire from '../../assets/files/questionnaire.js'
 ...
 */
export const switchContent = (forward, numberOfPages, currentPageIndex) => async dispatch => {
	// checks if the last page is reached and if yes, closes the modal
	if (
		forward && numberOfPages &&
		currentPageIndex &&
 ...
 */
export const setQuestionnaireItemMap = questionnaireItemMap => async dispatch => {
	dispatch({
...
export const getQuestionnaireSuccess = questionnaire => async dispatch => {
	dispatch({
		...
		questionnaire: config.appConfig.useLocalQuestionnaireInsteadOftheReceivedOne ? hardcodedTestQuestionnaire : questionnaire
	})
}
