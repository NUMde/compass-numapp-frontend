...
== Kiosk mode
The frontend client comes with a build-in kiosk mode. This means that the app can be set up in a way that it basically provides its own mock-backend to simulate a basic workflow within the app. The purpose is to demonstrate the capabilites of the NUM-App to new audiences.
