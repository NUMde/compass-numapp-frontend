...
The strings utilized in the app, as well as other aspects of the react-native client can be adjusted to your needs. The project comes with a set of configuration files that allow for quick and indiviual changes. The file responsible for the currently used strings is *`link:../../../../tree/main/src/config/textConfig.js[src/config/textConfig.js]`*
