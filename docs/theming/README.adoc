...
The visual style, as well as other aspects of the react-native client can be adjusted to your needs. The project comes with a set of configuration files that allow for quick and indiviual changes. The file responsible for the currently used theme is **`link:../main/src/theme/theme.js[src/theme/theme.js]`**.

WARNING: However, it is not recommended to alter this file directly, as it will aggravate the process of merging future updates into your own forked project. To ensure the updatability of your repository, another file is provided: **`link:../main/src/CUSTOMIZATION/customTheme.js[src/CUSTOMIZATION/customTheme.js]`**
**`link:../main/src/CUSTOMIZATION/customTheme.js[customTheme.js]`**. You can copy the content of the file **`link:../main/src/theme/theme.js[theme.js]`** into **`link:../main/src/CUSTOMIZATION/customTheme.js[customTheme.js]`**. There you can change the configuration values as you see fit. Both files contain explanations on how and where to copy the values.
...
When compoments import the theme, the regular configuration values from **`link:../main/src/theme/theme.js[theme.js]`** and the custom ones (**`link:../main/src/CUSTOMIZATION/customTheme.js[customTheme.js]`**) will be merged, with the custom ones overwriting the default values.

To access the theme you just need to import the file **`link:../main/src/config/configProvider.js[src/config/configProvider.js]`**. This is already done by default for existing components, but you have to do it manually for new ones should you want to add some. The imported object provides a property called 'theme', which in turn provides the different theme-properties:
...
IMPORTANT: Please use the provided file **`link:../main/src/CUSTOMIZATION/customTheme.js[customTheme.js]`** to update the default-style (like described above)
...
The default font-files can be found under **`link:../main/src/assets/fonts[src/assets/fonts]`**. Additional ones should be added to **`link:../main/src/CUSTOMIZATION/fonts[src/CUSTOMIZATION/fonts]`**.
...
The numeric parameters of the font-objects (like *'fontSize'*, *'lineHeight'*) are altered by the *'scaleFontsFkt'* function located in **`link:../main/src/config/appConfig.js[src/config/appConfig.js]`**. The reason for this is to have an additional method to dynamically alter font- & layout-sizes. The function itself can be altered using **`link:../main/src/CUSTOMIZATION/customAppConfig.js[src/CUSTOMIZATION/customAppConfig.js]`**.
...
Some aspects of the UI can be updated through the theme. It is possible to use a different logo and background for the banner and to allow the use of scroll-inidcators. Should you want to use a custom logo, then you must replace the appropriate file under **`link:../main/src/CUSTOMIZATION/images[src/CUSTOMIZATION/images]`**. The images that can be found here are the ones that replaces the default ones should you configure it (using the properties pictured in the image below).

.Updating the default-ui-settings in **`link:../main/src/CUSTOMIZATION/customTheme.js[customTheme.js]`**
[caption="Example: "]
==========================
...
WARNING: These files must always exist, as they are accessed during the build of the application. That is the reason why by default already two files exist in **`link:../main/src/CUSTOMIZATION/images[src/CUSTOMIZATION/images]`** - as placeholders.
...
TIP: When you replace the placeholders with your custom logo and logo-background into **`link:../main/src/CUSTOMIZATION/images[the custim-images-directory]`**, make sure that the correct filenames are used: *'logo.png'* and *'logoBackground.png'*

== Styles

Style properties that are used throughout the application can also be found in **`link:../main/src/theme/theme.js[theme.js]`** (and updated through **`link:../main/src/CUSTOMIZATION/customTheme.js[customTheme.js]`**). Here, the default look for buttons, labels, etc. is defined. The inidiviual components will use these values to determine their own style. Said values will reference the already defined colors (see the image below) to enforce a coherent theme, but you can change that to whatever floats your boat. Alongside single values like *'defaultBackgroundColor'*, combined values in form of objects are also defined here (to provide multiple style-attributes at once for selected elements like buttons).
