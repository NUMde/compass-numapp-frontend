...
Also there is currently a bug with some React-Native versions which causes the following error: 
```bash
Illegal callback invocation from native module react native push notifications
```
A new version (1.4.4) with a fix will be published soon. A patch has been provided: https://github.ibm.com/OHPC/gcs-app/tree/UMGCS-90/patches


To test the notifications on your phone, you have to clone [gcs-backend](https://github.ibm.com/OHPC/gcs-backend) as well and start the server locally. After starting the server you have to forward the localhost listening port to be available for the iPhone, e.g. expose with ngrok
```bash
ngrok http 8080
```
