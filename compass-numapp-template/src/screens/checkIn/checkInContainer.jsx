...
class CheckInContainer extends Component {
  ...
  initPush = async (subjectId) => {
    ...
    if (
      appConfig.reconnectOnEachUserUpdate ||
  ...
  getQuestionnaire = async () => {
    ...
    await loggedInClient
      ...
      .catch((error) => {
        Alert.alert(
          strings.generic.errorTitle,
          strings.generic.sendError,
          [
            {
  ...
  updateUserFail = (error) => {
    const { actions } = this.props;
    Alert.alert(
      strings.generic.info,
      strings.generic.updateError,
      [
        {
          text: strings.generic.ok,
          onPress: () => {
  ...
  updateUserSuccess = async (data) => {
    ...
    setTimeout(() => {
      ...
      if (lastQuestionnaireId && !noNewQuestionnaireAvailableYet) {
        ...
        if (
          appConfig.skipIncomingQuestionnaireCheck ||
  ...
  deleteLocalQuestionnaireData = async (message) => {
    ...
    setTimeout(() => {
      Alert.alert(
        strings.generic.info,
        message
          ? message + strings.generic.infoRemoval
          : strings.generic.infoRemoval,
        [
          {
            text: strings.generic.ok,
            onPress: async () => {
  ...
  exportAndUploadQuestionnaireResponseFail = (error) => {
    ...
    if (error.response.status === 409) {
      ...
      // the one the user is supposed to look at
      setTimeout(() => {
        this.deleteLocalQuestionnaireData(strings.generic.sendErrorTwoDevices);
      }, 0);
    } else {
      setTimeout(() => {
        Alert.alert(strings.generic.errorTitle, strings.generic.sendError);
      }, 0);
    }
  };
  ...
  exportAndUploadQuestionnaireResponseSuccess = async (response) => {
    ...
    setTimeout(() => {
      Alert.alert(strings.generic.successTitle, strings.generic.sendSuccess);
    }, 0);
  };
  ...
  exportAndUploadQuestionnaireResponse = () => {
    ...
    if (questionnaireItemMap && !questionnaireItemMap.done) {
      ...
      Alert.alert(
        strings.generic.info,
        strings.survey.sendUnfinishedMessageDenied,
        [
          {
            text: strings.generic.ok,
          },
    ...
    // triggers the call if confirmed
      ...
      Alert.alert(
        strings.generic.info,
        strings.survey.sendFinishedMessage,
        [
          {
            text: strings.survey.sendFinished,
            onPress: this.exportAndUploadQuestionnaireResponseStart,
          },
          {
            text: strings.generic.abort,
            style: 'cancel',
          },
        ],
        { cancelable: false },
  ...
  sendReportSuccess = (response) => {
    ...
    setTimeout(() => {
      Alert.alert(
        strings.reporting.symptoms_success_header,
        strings.reporting.symptoms_success_message,
        [
          {
            text: strings.reporting.symptoms_success_ok,
            onPress: () => {
              setTimeout(() => navigation.navigate('Home'), 0);
              this.updateUser();
            },
  ...
  sendReport = () => {
    ...
    if (user && user.additional_iterations_left) {
      ...
      Alert.alert(
        strings.generic.info,
        strings.generic.reportWhileInIteratedMode,
        [
          {
            text: strings.generic.ok,
          },
    ...
    // if there is a questionnaire available right now
      ...
      Alert.alert(
        strings.generic.info,
        strings.generic.reportWhileQuestionnaire,
        [
          {
            text: strings.generic.ok,
          },
    ...
    // shows an alert and sends out the report
      ...
      Alert.alert(
        ...
        strings.reporting.symptoms_question,
        [
          {
            text: strings.reporting.symptoms_yes,
            onPress: () => {
          ...
          {
            text: strings.reporting.symptoms_no,
            style: 'cancel',
          },
        ],
        { cancelable: false },
  ...
  deleteLocalDataAndLogout = () => {
    const { actions, navigation } = this.props;
    Alert.alert(
      strings.generic.warning,
      strings.generic.eraseLocalDataAtEndOfStudyText,
      [
        {
