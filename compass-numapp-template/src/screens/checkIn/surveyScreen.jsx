...
class SurveyScreen extends Component {
  ...
  getAccessibilityHint = (category) => {
    ...
    let hint = strings.accessibility.questionnaire.categoryCellHint;
    if (
      !questionnaireItemMap[category.linkId].done &&
      questionnaireItemMap[category.linkId].started
    ) {
      return (hint +=
        strings.accessibility.questionnaire.category +
        strings.accessibility.questionnaire.notFinished);
    }
    if (
      !questionnaireItemMap[category.linkId].done &&
      questionnaireItemMap[category.linkId].started
      ...
      return (hint +=
        strings.accessibility.questionnaire.category +
        strings.accessibility.questionnaire.notStarted);
    }
    if (questionnaireItemMap[category.linkId].done) {
      return (hint +=
        strings.accessibility.questionnaire.category +
        strings.accessibility.questionnaire.finished);
    }
    return (hint += '');
  };
  ...
  render() {
    ...
    return (
      <View style={{ ...localStyle.flexi, ...localStyle.wrapper }}>
        ...
        <Banner nav={navigation} title={strings.survey.title} />
...
localStyle = StyleSheet.create({
  ...
  listItemContainer: {
    ...
    borderBottomWidth: 1,
    backgroundColor: theme.values.SurveyItemBackgroundColor,
  },

  titleStyle: {
    ...theme.fonts.title2,
    color: theme.values.SurveyItemTitleColor,
  },
});
